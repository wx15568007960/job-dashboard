...
const FIELDS=['score','grade','title','company','category','location','kanto','employ','salary','visa','visaNote','innov','source','url','bd','note'];
const idx={}; store.jobs.forEach(j=>idx[(j.company||'')+'||'+(j.title||'')]=j);
let added=0,updated=0;
JOBS.forEach(j=>{const k=(j.company||'')+'||'+(j.title||'');
 if(idx[k]){const e=idx[k]; if(e.lastSeen!==today)e.seenCount=(e.seenCount||1)+1; e.lastSeen=today;
